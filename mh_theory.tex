...
%= \sum_{i=1}^{N_E^2}\left[ \int_{\partial T_2^i \cap \Omega_{1}} \sigma_2 (p_{1} - p_{2})^2 \dd s \right] + \sum_{i=1}^{N_E^3}\left[ \int_{\partial T_3^i \cap \Omega_{2}} \sigma_3 p_{2}^2 \dd s \right].
%\end{equation}
...
\noteJB{Exactly. The Poincare ineq. is necessary for the ellipticity. I do not know a version of the 
Lax-Milgram for the Neumann only case. I rather introduced the zero mean condition to stay within the standard theory. 
}
\begin{equation*}
...
\noteJB{This means that DOFs of $W_k$ are means over elements and means over edges.}
...
\noteJB{This is incorect since we can not have the test function for just single face. In fact this is a subtile part of the proof.
The test function $\phi_e$ (I rather use $\nu$.) is any member of $\Lambda_k$ such that for any edge/face $e$ and any connected element $T$:
\[
 \int_{e} 
    \left( \nu - \phi|_{T\cap e}\right) \dd s = 0
\]
The space $\Lambda_k$ has to be introduced for the face set $\mathcal F_k$. In particular for an edge $e = T \cap T'$ the functions
$\phi|_{e\cap T}$, $\phi|_{e\cap T'}$, $\nu|_e$ could be different, but have a coomon mean over the $e$. Then
\[
    \int_{e} (\nu|_{e}  - \phi|_{T\cap e})\vc v \cdot \vc n \dd s = 0
\]
since $\vc v \cdot \vc n$ is constant on $e$.}
\noteJB{We have to use better notation. At least we can rename $\phi$ to $r$ and $\phi_e$ to $\nu$. 
In the mamern stub I have used the ring accent for the trace spaces and variables.}
